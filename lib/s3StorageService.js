...
class S3StorageService {
    ...
    getBaseEndpointUrl (bucket) {
        ...
        if (this._config.urlStyle == 'virtualHost') return this.getVirtualHostedStyleBaseEndpointUrl(bucket);
        else return this.getPathStyleBaseEndpointUrl(bucket);
    }
     ...
     * @returns {String}
     */
    getPathStyleBaseEndpointUrl (bucket) {
     ...
     * @returns {String}
     */
    getVirtualHostedStyleBaseEndpointUrl (bucket) {
        let urlPredicate = this._config.baseUrlWithoutBucket.replace('http://', `http://${bucket}.`);
        urlPredicate = urlPredicate.replace('https://', `https://${bucket}.`);
        return urlPredicate + (this._config.keyPrefix ? `/${this._config.keyPrefix}` : '');
    }
     ...
     * @returns {Promise.<Data,Error>}
     */
    deleteObjectsWithPrefix (params) {
        if (this._config.readOnly) {
            return Promise.reject(new Error('Readonly service - no CUD operations allowed'))
        } else {
            params.Prefix = this.getKeyWithKeyPrefix(params.Prefix);
            return this.listObjects({ Bucket: this.getBucket(), Prefix: params.Prefix })
            .then(data => {
                const toDelete = data.Contents.map(content => {
    ...
    /**
     * @name S3StorageServiceWebsiteConfiguration
     * @type Object
     * @property {{ Key: {String} }} ErrorDocument
     * @property {{ Suffix: {String} }} IndexDocument
     * @property {{ HostName: {String}, Protocol: 'http|https' }} RedirectAllRequestsTo
     * @property {[S3StorageServiceWebsiteRoutingRule]} RoutingRules
     */

    /**
     * @name S3StorageServiceWebsiteRoutingRule
     * @type Object
     * @property {Object} Redirect
     * @property {String} Redirect.HostName
     * @property {String} Redirect.HttpRedirectCode
     * @property {'http|https'} Redirect.Protocol
     * @property {String} Redirect.ReplaceKeyPrefixWith
     * @property {String} Redirect.ReplaceKeyWith
     * @property {Object} Condition
     * @property {String} Condition.HttpErrorCodeReturnedEquals
     * @property {String} Condition.KeyPrefixEquals
     */

    /**
     * Wrapper for s3 putBucketWebsite - http://docs.aws.amazon.com/AWSJavaScriptSDK/latest/AWS/S3.html#putBucketWebsite-property
     *
     * @param {S3StorageServiceWebsiteConfiguration} params
     * @returns {Promise.<Data,Error>}
     */
